...
use proof_eval::{evaluate, types::{EvalInput, EvalOutput}};
...
pub fn compare(
    ...
    diff_decimal("amount",       spec_output.amount,      system_output.amount,      &mut diffs);
    diff_decimal("rate_applied", spec_output.rate_applied, system_output.rate_applied, &mut diffs);
    diff_str("applied_tier", &spec_output.applied_tier,  &system_output.applied_tier, &mut diffs);
...
fn diff_decimal(field: &str, spec: Option<Decimal>, sys: Option<Decimal>, out: &mut Vec<FieldDiff>) {
    match (spec, sys) {
        (Some(s), Some(y)) if s != y => {
            let delta = y - s;
            let pct   = if s.is_zero() { None } else {
        ...
        (Some(s), None) => out.push(FieldDiff {
            field: field.into(), spec_value: s.to_string(),
            system_value: "(missing)".into(), delta: None, delta_pct: None,
        }),
        (None, Some(y)) => out.push(FieldDiff {
            field: field.into(), spec_value: "(none)".into(),
            system_value: y.to_string(), delta: None, delta_pct: None,
        }),
        _ => {}
    }
}
