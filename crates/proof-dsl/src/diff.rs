...
pub enum DiffItem {
    BaseRateChanged     { old: Decimal, new: Decimal, delta: Decimal },
    TierThresholdChanged { tier_index: usize, old_threshold: Decimal, new_threshold: Decimal },
    TierRateChanged     { tier_index: usize, old_rate: String, new_rate: String },
    TierAdded           { tier_index: usize },
    TierRemoved         { tier_index: usize },
    PromotionalRateChanged { old: String, new: String },
    ObligationChanged   { field: String, old: String, new: String },
}
...
fn fmt_days(d: Option<u32>) -> String {
    d.map(|v| format!("{} days", v)).unwrap_or_else(|| "none".into())
}