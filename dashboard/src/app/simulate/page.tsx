...
const VERDICT_CONFIG = {
  DeployClean:       { label: 'Deploy — no concerns',   color: '#10B981', bg: '#ECFDF5', border: '#A7F3D0' },
  DeployWithReview:  { label: 'Deploy with review',     color: '#F59E0B', bg: '#FFFBEB', border: '#FDE68A' },
  DoNotDeploy:       { label: 'Do not deploy',          color: '#EF4444', bg: '#FEF2F2', border: '#FECACA' },
}
  ...
  return (
    <div className="p-8 min-h-full flex flex-col">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Portfolio simulation</h1>
          <p className="text-sm text-slate-500 mt-0.5">Replay your portfolio through a proposed spec change before deploying.</p>
        </div>
        <button
          onClick={runSim}
          disabled={loading}
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            <span className="text-sm font-semibold text-slate-900">New spec (.proof)</span>
            <span className="text-xs font-semibold text-blue-600 bg-blue-50 border border-blue-100 px-2.5 py-1 rounded-full">Proposed</span>
          </div>
          <textarea
            value={newSpecText}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            <span className="text-sm font-semibold text-slate-900">Portfolio</span>
            <span className="text-xs font-semibold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-full">JSON array of events</span>
          </div>
          <textarea
            value={portfolioText}
      ...
      {report && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              { label: 'Total customers',   value: report.customers_total,   color: '#3B82F6' },
              { label: 'Worse off',         value: report.customers_worse,   color: '#EF4444' },
              { label: 'Better off',        value: report.customers_better,  color: '#10B981' },
              { label: 'Unaffected',        value: report.customers_neutral, color: '#94A3B8' },
          ...
          {/* Monthly impact + regulatory flags */}
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
              <div className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-3">Financial impact</div>
              <div className="space-y-2">
                {[
                  { label: 'Daily aggregate',  value: `£${report.daily_delta}` },
                  { label: 'Monthly aggregate',value: `£${report.monthly_delta}` },
                  ...(report.avg_delta_worse ? [{ label: 'Avg impact per affected customer', value: `£${report.avg_delta_worse}/day` }] : []),
                ].map(({ label, value }) => (
            ...
            {report.regulatory_flags.length > 0 ? (
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                <div className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-3">Regulatory flags</div>
                  ...
                  {report.regulatory_flags.map((flag, i) => {
                    ...
                    return (
                      <div key={i} className="flex gap-3 items-start">
                        <div className={`w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${isBlock ? 'bg-red-50' : isReview ? 'bg-amber-50' : 'bg-blue-50'}`}>
                          <span className={`text-xs font-bold ${isBlock ? 'text-red-600' : isReview ? 'text-amber-600' : 'text-blue-600'}`}>
                            {isBlock ? '✗' : isReview ? '!' : 'i'}
                          </span>
                        </div>
                        <div>
                          <div className="text-sm font-semibold text-slate-900">{flag.rule}</div>
                          <div className="text-xs text-slate-500 mt-0.5">{flag.description}</div>
                          <div className="text-xs text-slate-500">{flag.action}</div>
                          {flag.notice_days && (
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-semibold text-slate-900">No regulatory flags</div>
                  <div className="text-xs text-slate-500 mt-0.5">This change does not trigger any FCA obligations.</div>
                </div>
              </div>
            )}
          </div>
